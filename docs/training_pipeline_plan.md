...
| **6. Two-Tower (S1 加速) + MMR (F2)**（stretch）🟡 | 5/17 – 5/18        | 6.1 base ✅ commit `1bcce05` (Recall@200 0.224)；6.1b in-batch softmax ✅ commit `49a9939` (退化)；6.1c **Pipeline C hard-filter+DeepFM** ✅ commit `<6c_HASH>` (full-pool Recall@10=0.018, **15× pipeline A**, demo 主流程候选)；6.2 MMR 待开始 (defer) | Haobo                  | 2d   |
