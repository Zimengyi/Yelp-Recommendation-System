...
> **关联**：`PRD_v1_section3.3_Model_v2.md` §3.3.6 训练 Protocol / `recommender_training_explainer.md` §2 训练数据
...
| **5. DeepFM 主训练 + sweep**                    | 5/14 – 5/16        | DeepFM 选定 config + ablation 结果              | Haobo                  | 3d   |
...
## §2 Phase 2 — EDA（5/6 – 5/11，与 Phase 1 并行）
...
| **3.2.3** | 输出 `data/features/item_features.parquet`（行数 ≈ 13K 餐厅 + 1 OOV）|
...
## §4 Phase 4 — Baseline 模型（5/12 – 5/13）
...
## §5 Phase 5 — DeepFM 主训练 + Hyperparam Sweep（5/14 – 5/16）
...
### 6.1 Two-Tower (DSSM) 召回

| 步骤 | 详细 |
|---|---|
| **6.1.1** | 写 `notebooks/06_two_tower.ipynb`，用 PyTorch 实现 User Tower + Item Tower（各 MLP[64,32]）|
| **6.1.2** | 训练目标：cosine(u, v) 拟合 BCE label，用 in-batch negative sampling |
| **6.1.3** | 离线预计算所有 13K 餐厅的 item_vec → 建 FAISS IVF 索引 |
| **6.1.4** | 评测：召回 top-200 中是否包含 test 集真正例的比例（Recall@200）|

**时间预估**：8-10 小时
**期望**：Recall@200 ≥ 0.75（即 75% 真正例进入 top-200 召回池）
**验收标准**：
- [ ] FAISS 索引建成，单次 query < 50ms
