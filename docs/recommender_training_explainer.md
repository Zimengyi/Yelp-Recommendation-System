...
### 1.2 v2.2 架构：单 DeepFM + 双路召回

> **v2.2 核心升级**（2026-05-06）：系统从 v2.1 的单路「Two-Tower → DeepFM → MMR → LLM」升级为**双路召回 + 共享精排**架构。用户输入先判断是否带有自然语言 utterance，据此分流到两条不同的召回路径，最终汇合到同一个 DeepFM 精排模型。这是 Conversational Recommendation System (CRS) 学术范式（Lei et al. 2020 / Sun & Zhang 2018）的标准实现：**有 query 走意图过滤，无 query 走 dense retrieval，精排统一**。
...
```
  ...
  │  (用户进入屏，无 query)  │         │  (对话 / 旅行规划，有 query)│
  │                        │         │                           │
  │  Two-Tower DSSM 召回   │         │  Step 1: LLM intent       │
  │  User Tower MLP        │         │  extraction → IntentJSON  │
  │  Item Tower MLP        │         │  {cuisine, price,         │
  │  cosine + FAISS ANN    │         │   dietary, dist, ...}     │
  │  over ~13K restaurants │         │                           │
  │  → top-200             │         │  Step 2: Hard filter      │
  │  (basic geo+time       │         │  SQL-style WHERE clause   │
  │   pre-filter)          │         │  → top-100                │
  └────────────┬───────────┘         └─────────────┬────────────┘
               ...
               └──────────────────┬────────────────┘
                                  ▼
               ┌──────────────────────────────────┐
               │  Layer 2 — 精排 (Shared Ranking)   │
               │  DeepFM（单一训练，双路复用）        │
               │                                   │
               │  26 features:                     │
               │   • User (8) / Item (9)            │
               │   • Context (9, 含 trip features)  │
               │  → CTR score per candidate        │
               │  → top-10 (F1) / top-50 (F2 trip) │
               └──────────────────┬────────────────┘
                                  │
                       ┌──────────┴──────────┐
                       │   F2 Trip 场景?      │
...
**双路设计的搜索域类比**：
- S1 push 路（无 utterance）= 搜索里的 "exploratory browsing"——没有 query terms，必须靠 dense embedding 做 ANN 检索，类比倒排拉链找不到锚点时退到语义检索；
- S2/S6 CRS 路（有 utterance）= 搜索里的 "keyword search"——LLM 从 utterance 提取 intent，作 SQL-style 硬过滤，类比倒排拉链 + Boolean 约束；
- DeepFM 精排层 = 两条路的共同 LTR ranker，单次训练，双路复用。
...
>
> 以下 22 字段中，我们的 §3.3.3 8 个 User Features 直接派生 5 个（user_id / average_stars → avg_rating_given / review_count → review_count_log / yelping_since → days_active / elite → elite_flag），另外 3 个（mean_distance_traveled / fav_cuisine_emb / price_tolerance_avg）需要从 review.json + business.json 聚合计算（详见 §2.2 伪代码）。`compliment_*` 11 个字段我们暂不使用，可作 future work（衡量用户社交活跃度的辅助信号）。
>
> ```
...
**这就是一条训练样本**。~7M reviews → ~6.5M labeled training samples（其中 rating ≥ 4 的 explicit positive ~4.5M，rating < 4 的 weak/negative ~2M）。
...
> **范围说明**：以下架构仅适用于 **S1 push 推荐路径**（用户进入屏、无 utterance 的场景）。S2/S6 conversational 路径走 LLM intent extraction + hard filter 召回，**不依赖此 Layer 1**——Two-Tower 对 S2/S6 路的 recall 质量无任何影响。

**目的**：从 ~13K 目标城市餐厅中快速找到 top-200 候选，避免对全量商家跑 DeepFM forward pass（O(13K) 直接精排虽可行，Two-Tower + FAISS ANN 可将延迟从 ~1s 压到 50ms 以下）。
...
| `fav_cuisine_emb` | top-3 cuisine pooling | 优先：`accumulated_prefs.cuisine_likes` 即时合成；fallback：全局 prior | 与 §6.2 / §6.3 部署状态联动（详见本文 deploy 状态章节） |
...
### Q: 我们没有用户数据，怎么训推荐？
A: 用 Yelp ~2M 历史用户（实测 1,987,897）作为训练 proxy。每条 review (rating ≥ 4) 是一条 positive 监督信号，加 4× 同城未访问商家作 negative。训练 DeepFM 学协同 + 内容信号。**Yelp 历史用户在训练阶段=我们的"用户"**，Taste hunter 真实用户在部署时走冷启动 + 对话累积偏好路径。
...
A: 性能问题，且**双塔仅服务于 S1 push 路**（无 utterance 的用户进入屏场景）。S2/S6 conversational 路通过 LLM intent extraction → hard filter 召回（SQL-style WHERE 约束），不依赖双塔——这两条路从召回起就完全独立，汇合到共享 DeepFM 精排层。对于 S1 路，~13K 目标城市餐厅直跑 DeepFM forward pass 约 1.3 秒，两层架构压到 50ms。但如时间紧张，S1 路可降级为"地理 + 时间硬过滤"代替双塔（详见 §3.3.4 降级策略），S2/S6 路完全不受影响，demo 照常可用。
